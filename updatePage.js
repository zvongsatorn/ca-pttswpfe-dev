...
export default function MKDDashboardPage({ params }: { params: { mkdId: string } }) {
    ...
    const fetchData = async () => {
      try {
        ...
        const [detailRes, dashRes] = await Promise.all([
          fetch(\`/api/mkd/\${params.mkdId}/details\`).then(r => r.json()),
          fetch(\`/api/mkd/\${params.mkdId}/dashboard\`).then(r => r.json())
        ]);
  ...
  const handleSaveRate = async () => {
      try {
          const payload = Object.entries(editRateValues).map(([year, amount]) => ({
              id: Number(params.mkdId),
              year: year.toString(),
              amount: Number(amount)
          }));
          const res = await fetch(\`/api/mkd/\${params.mkdId}/dashboard/rate\`, {
              method: 'POST',
...
fs.writeFileSync('/Users/zia/Projects/pttswp/frontend/src/app/mkd/dashboard/[mkdId]/page.tsx', content);
