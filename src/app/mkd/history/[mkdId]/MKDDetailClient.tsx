...
export default function MKDDetailClient({ mkdId, token, currentUser, initialData, masterKeys }: MKDDetailClientProps) {
    ...
    const [note, setNote] = useState(initialData.header?.Remark || '');
    ...
    const [mkdData, setMkdData] = useState(() => {
        ...
        return mainKeys.map(mk => {
            ...
            // If index type but no subs yet, or uniform
    ...
    const formatYearBE = (year: string | number) => {
        const yNum = parseInt(year.toString());
        return yNum < 2400 ? (yNum + 543).toString() : yNum.toString();
    };

    const getYearStyle = (year: string) => {
        const yNum = parseInt(year);
        const displayYear = parseInt(formatYearBE(yNum));
        const targetEff = parseInt(formatYearBE(effectiveYear));
    ...
    const handleSaveNote = async () => {
        try {
            setLoading(true);
            await updateNoteAction(mkdId, note, token);
            message.success('บันทึกหมายเหตุสำเร็จ');
        } catch (error) {
            message.error('เกิดข้อผิดพลาดในการบันทึกหมายเหตุ');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleRequestApprove = () => {
        ...
        if (totalWeight > 100) {
            modal.warning({
                title: 'Weight รวมเกิน 100%',
                content: `รวม Weight (%) ทั้งหมดของรายการหลักต้องไม่เกิน 100% (ปัจจุบัน: ${totalWeight}%) กรุณาตรวจสอบและแก้ไขให้ถูกต้องก่อนส่งอนุมัติ`,
            });
            return;
        }
        ...
        modal.confirm({
            ...
            content: 'คุณต้องการส่งเอกสารนี้เพื่อขออนุมัติใช่หรือไม่?',
            onOk: async () => {
                try {
                    setLoading(true);
                    const res = await requestApproveAction(mkdId, currentUser.employeeID, token);
                    if (res?.success) {
                        message.success('ส่งคำขออนุมัติเรียบร้อยแล้ว');
                        router.push('/mkd/history');
                    } else {
                        message.error(res?.message || 'เกิดข้อผิดพลาดในการส่งคำขออนุมัติ');
                    }
                } catch (error) {
    ...
    const handleCancel = () => {
        modal.confirm({
            ...
            content: 'คุณต้องการยกเลิกเอกสารนี้ใช่หรือไม่? หลังจากยกเลิกแล้วจะไม่สามารถแก้ไขได้อีก',
            okType: 'danger',
            onOk: async () => {
                try {
                    setLoading(true);
                    await updateStatusAction(mkdId, 0, currentUser.employeeID, token);
                    message.success('ยกเลิกเอกสารเรียบร้อยแล้ว');
                    router.push('/mkd/history');
                } catch (error) {
                    message.error('เกิดข้อผิดพลาด');
                } finally {
    ...
    const manpowerColumns = [
        ...
        },
        {
            ...
            width: 80,
            render: (type: string) => (
                <Tag color={type === 'index' ? 'blue' : 'cyan'} className="capitalize">
                    {type}
                </Tag>
            )
        },
        {
            ...
            key: 'definition',
            render: (_, record: any) => record.subItems[0]?.definition || '-'
        },
        {
            title: 'Weight(%)',
            dataIndex: 'weight',
        ...
        ...allYears.map(year => {
            ...
            return {
                title: `${formatYearBE(year)}${style.suffix}`,
                key: year,
                align: 'right' as const,
                width: 100,
                className: style.color === 'purple' ? 'bg-purple-50/30' : '',
                render: (_, record: any) => {
                    ...
                    return <Text color={style.color}>{formatNumber(sum)}</Text>;
                }
            };
        }),
        {
                ...
                <Space>
                    {!isReadOnly && (
                        <Button 
                            ...
                            onClick={() => message.info('Functionality coming soon')}
                        />
                    )}
                    <Button 
                        type="text" 
            ...
            <div className="max-w-[1600px] mx-auto">
                ...
                <Card className="mb-6 shadow-sm border-0 bg-white" bodyStyle={{ padding: '24px' }}>
                    ...
                    <div className="flex gap-4">
                        <Tag color={isReadOnly ? 'warning' : 'processing'} className="px-4 py-1 text-sm font-medium">
                            Status: {header.StatusName || '-'}
                        </Tag>
                        <Tag color="cyan" className="px-4 py-1 text-sm font-medium">
                            Effective Year: {formatYearBE(effectiveYear)}
                        </Tag>
                    </div>
                </Card>
                ...
                <Card className="shadow-sm border-0" bodyStyle={{ padding: 0 }}>
                    <Tabs
                        ...
                        items={[
                            {
                                key: 'manpower',
                                label: <Space><FileTextOutlined /> MANPOWER KEY DRIVER</Space>,
                                children: (
                                    <div className="p-4">
                            ...
                            {
                                key: 'summary',
                                label: <Space><FileExcelOutlined /> SUMMARY</Space>,
                                children: (
                                    <div className="p-8 text-center">
                                        ...
                                        <Button 
                                            ...
                                            onClick={() => message.success('Export logic ready')}
                                        >
                                            Export to Excel
                                        </Button>
                                    </div>
                                        ...
                                        <div className="flex justify-between items-center mb-6">
                                            ...
                                            {!isReadOnly && (
                                                <Upload showUploadList={false}>
                                                    <Button icon={<UploadOutlined />} type="primary">Upload New File</Button>
                                                </Upload>
                                            )}
                                        </div>
                                        <List
                                                    ...
                                                    <Card 
                                                        ...
                                                        actions={[
                                                            <Tooltip title="View File">
                                                                <FileTextOutlined key="view" className="text-blue-500" />
                                                            </Tooltip>,
                                                            !isReadOnly && (
