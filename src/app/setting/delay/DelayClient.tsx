...
import { DelayRetirementDataType } from '@/services/delayService';
import { saveDelayAction, deleteDelayAction } from './actions';

interface DelayClientProps {
    initialData: DelayRetirementDataType[];
    employeeOptions: any[];
}

export default function DelayClient({ initialData, employeeOptions }: DelayClientProps) {
    ...
    const [tableData, setTableData] = useState<DelayRetirementDataType[]>(initialData);
    ...
    const handleDelete = async (key: string) => {
        const res = await deleteDelayAction(key);
        if (res.success) {
    ...
    const handleSave = async () => {
        try {
            ...
            const res = await saveDelayAction(values);
    ...
    const columns: ColumnsType<DelayRetirementDataType> = [
        ...
        },
        {
            ...
            render: (status: number) => (
                <Tag color={status === 1 ? 'green' : 'red'} className="rounded-full px-4 border-none font-bold">
                    {status === 1 ? 'ใช้งาน' : 'ยกเลิก'}
                </Tag>
            ),
        },
        {
    ...
    return (
        <div className="w-full bg-slate-50 min-h-screen p-6">
                ...
                <div className="flex flex-wrap items-end justify-between gap-4">
                    <div className="flex flex-wrap items-end gap-6">
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <Input
                                placeholder="รหัสพนักงาน หรือ ตำแหน่ง..."
                    ...
                    <Button
                        ...
                        onClick={handleAdd}
                        size="large"
                        className="bg-blue-600 hover:bg-blue-700 px-8 font-bold shadow-lg shadow-blue-100 rounded-xl"
                    >
                        เพิ่มรายการ
                    </Button>
                </div>
            ...
            <Modal
                ...
                cancelButtonProps={{ className: 'px-8 rounded-lg' }}
                width={550}
            >
                <div className="py-6">
                    <Form form={form} layout="vertical" requiredMark={false}>
                        <Form.Item
                            name="EmployeeID"
                            label={<span className="font-bold text-slate-600 uppercase text-xs tracking-wider">พนักงาน</span>}
                        ...
                        >
                            <Select
                                ...
                                onChange={(val) => {
                                    ...
                                    form.setFieldsValue({ PosName: opt?.position });
                                }}
                            />
                        </Form.Item>
                        ...
                        <Form.Item
                            name="PosName"
                            label={<span className="font-bold text-slate-600 uppercase text-xs tracking-wider">ตำแหน่ง</span>}
                        >
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <Form.Item
                                name="DelayStatus"
                                label={<span className="font-bold text-slate-600 uppercase text-xs tracking-wider">สถานะ</span>}
                                rules={[{ required: true }]}
                            >
                                <Select
                                    size="large"
                                    options={[
                                        { value: 1, label: 'ใช้งาน (Active)' },
                                        { value: 0, label: 'ยกเลิก (Inactive)' },
                                    ]}
                                />
                            </Form.Item>
                        </div>
