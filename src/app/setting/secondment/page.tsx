...
import { getSecondmentPools, getParentUnits, getUnitCombo, createSecondmentPool, updateSecondmentPool, UnitCombo, SecondmentPool } from '@/services/secondmentService';
...
function SecondmentContent() {
    ...
    const [allUnits, setAllUnits] = useState<UnitCombo[]>([]);
    const [parentUnits, setParentUnits] = useState<UnitCombo[]>([]);
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            ...
            if (parentsRes?.data) setParentUnits(parentsRes.data);
            if (allUnitsRes?.data) setAllUnits(allUnitsRes.data);
        };
        fetchInitialData();
    }, [token]);
        ...
        <div className="w-full bg-slate-50 min-h-screen p-6">
            ...
            <Card className="mb-8 border-slate-200">
                <div className="flex flex-col md:flex-row justify-center items-center gap-6 py-4">
                    <div className="flex items-center gap-4">
                        ...
                        <Select placeholder="-- เลือกหน่วยงานทั้งหมด --" className="w-[500px]" size="large" showSearch allowClear value={selectedParent} onChange={setSelectedParent} options={allUnits.map(u => ({ label: u.UnitText, value: u.OrgUnitNo }))} />
            ...
            {selectedParent && (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <Table columns={columns} dataSource={poolUnits} loading={loading} pagination={false} bordered rowKey="OrgUnitNo" className="shadow-sm rounded-xl overflow-hidden border-slate-100" />
                </div>
            )}
            ...
            <Modal title={<div className="font-bold text-lg border-b pb-3">เพิ่มหน่วยงานเข้า Pool</div>} open={isAddModalOpen} onOk={handleAddUnit} onCancel={() => setIsAddModalOpen(false)} okText="เพิ่มเข้า Pool" okButtonProps={{ className: "bg-emerald-500 font-bold" }} width={600}>
                <div className="py-8">
                    ...
                    <Select placeholder="พิมพ์เพื่อค้นหาหน่วยงาน..." className="w-full" size="large" showSearch value={selectedUnitToAdd} onChange={setSelectedUnitToAdd} options={allUnits.map(u => ({ label: u.UnitText, value: u.OrgUnitNo }))} />
                </div>
            </Modal>
            ...
            <Modal title={<div className="font-bold text-lg border-b pb-3">หน่วยงานที่มีการเพิ่ม Secondment Pool</div>} open={isOrgModalOpen} onCancel={() => setIsOrgModalOpen(false)} footer={[<Button key="close" onClick={() => setIsOrgModalOpen(false)} className="px-8 font-bold">ปิด</Button>]} width={700}>
                <div className="py-4">
                    <Table size="middle" pagination={{ pageSize: 8 }} dataSource={parentUnits} rowKey="OrgUnitNo" columns={[
