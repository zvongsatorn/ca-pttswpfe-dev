...
import { toggleMenuRightAction } from './actions';
import { fetchMenuRightsByRole } from '@/services/menuService';
...
interface MenuRightClientProps {
    roles: UserGroup[];
    token: string;
}

export default function MenuRightClient({ roles, token }: MenuRightClientProps) {
    ...
    const handleToggleRight = async (menuID: number, hasRight: boolean) => {
        ...
        const res = await toggleMenuRightAction({
            userGroupRole: selectedRole,
            menuID,
            hasRight
        }, token);
    ...
    const columns: ColumnsType<MenuItem> = [
        {
            ...
            render: (text, record) => (
                <span>
                    {record.MenuIcon && <i className={`${record.MenuIcon} mr-2`} />} 
                    {text}
                </span>
            )
        },
        {
            ...
            render: (_, record) => (
                <Switch 
                    checked={record.hasRight} 
                    onChange={(checked) => handleToggleRight(record.MenuID, checked)}
