...
export default function UserGroupsClient({ initialUserGroups, currentUser, token }: UserGroupsClientProps) {
    ...
    const handleAddLevel = async () => {
        ...
        setLoading(true);
        try {
            const res = await addLevelAction(selectedGroup.userGroupNo, selectedLevelToAdd, currentUser.employeeID, token);
            if (res.success) {
    ...
    const handleDeleteLevel = useCallback((levelGroupNo: string) => {
        ...
        modal.confirm({
            ...
            onOk: async () => {
                const res = await deleteLevelAction(selectedGroup.userGroupNo, levelGroupNo, currentUser.employeeID, token);
                if (res.success) {
    ...
    }, [selectedGroup, modal, currentUser.employeeID, token, message, openLevelModal]);
    ...
    const handleAddUser = async () => {
        ...
        setLoading(true);
        try {
            const res = await addMemberAction(selectedGroup.userGroupNo, selectedUserToAdd, currentUser.employeeID, token);
            if (res.success) {
    ...
    const handleDeleteUser = useCallback((employeeID: string) => {
        ...
        modal.confirm({
            title: 'ต้องการยืนยันใช่หรือไม่?',
            onOk: async () => {
                const res = await deleteMemberAction(selectedGroup.userGroupNo, employeeID, currentUser.employeeID, token);
                if (res.success) {
                    message.success("ลบข้อมูลเรียบร้อย");
                    await openUserModal(selectedGroup);
                } else {
    ...
    }, [selectedGroup, modal, currentUser.employeeID, token, message, openUserModal]);
