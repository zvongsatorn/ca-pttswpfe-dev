...
import { Select, Button, Modal, App, Card, Space, Divider } from 'antd';
import { Users, Copy, ArrowRight } from 'lucide-react';
import { InfoCircleOutlined } from '@ant-design/icons';
import { copyOrgRightsAction } from './actions';
import { fetchUserGroupMembers } from '@/services/userRightService';

interface CopyrightClientProps {
    initialUserGroups: { value: string; label: string }[];
    token: string;
    currentUser: any;
}

export default function CopyrightClient({ initialUserGroups, token, currentUser }: CopyrightClientProps) {
    ...
    const handleGroupChange = useCallback(async (value: string) => {
        ...
        try {
            ...
            if (res && Array.isArray(res)) {
                setUsersInGroup(res.map((u: any) => ({ value: u.employeeID, label: `${u.employeeID} - ${u.nameAll}` })));
            } else {
    ...
    const handleSubmit = async () => {
        if (!selectedGroup) {
            notification.warning({ message: 'คำเตือน', description: 'กรุณาเลือกกลุ่มผู้ใช้งาน' });
            return;
        }
        if (!selectedUserFrom || !selectedUserTo) {
            notification.warning({ message: 'คำเตือน', description: 'กรุณาเลือกผู้ใช้งานให้ครบถ้วน' });
            return;
        }
        if (selectedUserFrom === selectedUserTo) {
            notification.warning({ message: 'คำเตือน', description: 'กรุณาเลือกผู้ใช้งานที่ต่างกัน' });
            return;
        }
        ...
        modal.confirm({
            ...
            onOk: async () => {
                setLoading(true);
                try {
                    const res = await copyOrgRightsAction({
    ...
    return (
        <div className="w-full bg-slate-50 min-h-screen p-6">
            ...
            <Card className="max-w-4xl mx-auto border-slate-200 shadow-sm overflow-hidden" bodyStyle={{ padding: 0 }}>
                <div className="bg-blue-50/50 p-6 border-b border-blue-100 flex items-start gap-4">
                    <div className="bg-blue-100 p-2 rounded-lg">
                        <InfoCircleOutlined style={{ fontSize: '20px', color: '#2563eb' }} />
                    </div>
                    <div>
                        <h3 className="font-bold text-slate-800 m-0">คำแนะนำการใช้งาน</h3>
                        <p className="text-slate-500 text-sm mt-1 mb-0 leading-relaxed">
                            ระบบจะทำการคัดลอกสิทธิ์การมองเห็นหน่วยงาน (Organization Rights) จากผู้ใช้งานต้นทางไปยังผู้ใช้งานปลายทาง 
                            เพื่อให้ผู้ใช้งานปลายทางมีสิทธิ์จัดการข้อมูลในสายงานเดียวกัน
                        </p>
                    </div>
                </div>
                ...
                <div className="p-8">
                    <div className="grid grid-cols-1 gap-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center relative">
                            ...
                            {/* Arrow icon between for larger screens */}
